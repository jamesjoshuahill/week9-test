...
def select_elements_starting_with_a(array)
  array.select { |element| element.first_letter == 'a' }
end

# keep only the elements that start with a vowel
def select_elements_starting_with_vowel(array)
  array.select { |element| %w(a e i o u).include?(element.first_letter) }
end

class String

  def first_letter
    self.downcase.chars.first
  end
...
def shortest_word_in_array(array)
  array.inject do |shortest, element|
    element.length < shortest.length ? element : shortest
  end
end

# return the shortest word in an array
def longest_word_in_array(array)
  array.inject do |longest, element|
    element.length > longest.length ? element : longest
  end
end
...
def titleize_a_string(string)
  ...
  words.map! { |word| %w(a and the).include?(word) ? word : word.capitalize }.join(' ')
end
...
def word_count_a_file(file_path)
  ...
  file.each_line { |line| words += line.split.count }
  file.close
  words
end
...
def count_words_of_each_length_in_a_file(file_path)
  ...
  file.each_line do |line|
    line.gsub(/\W+/, ' ').split.map(&:length).each do |word_length|
      if word_lengths[word_length]
